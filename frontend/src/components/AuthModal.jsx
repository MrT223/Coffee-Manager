...
export default function AuthModal({ isOpen, onClose, onLoginSuccess }) {
  ...
  const handleRequestOtp = async () => {
    ...
    try {
        ...
        // Xóa reCAPTCHA sau khi đã gửi mã thành công
  ...
  const handleSubmit = async (e) => {
    ...
    // Forgot Password: Step 1 - check phone exists then request OTP
    if (isForgotPassword && !showOtpStep) {
        setLoading(true);
        try {
            await axios.post(`${API_BASE}/auth/check-phone`, { 
    ...
    // Register: Step 1 - request OTP (chưa nhập OTP)
  ...
  useEffect(() => {
    if (!isOpen) {
      resetForm();
    } else if ((!isLogin || isForgotPassword) && !showOtpStep) {
  ...
  }, [isOpen, isLogin, showOtpStep, isForgotPassword]);
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  {!showOtpStep ? (
                    <>
                      ...
                      {/* Số điện thoại - hiện ở mọi mode */}
                      <div className="relative">
                        <Phone className="absolute left-3 top-1/2 -translate-y-1/2 size-5 text-white/30" />
                        <input name="username" type={isLogin && !isForgotPassword ? "text" : "tel"} value={formData.username} onChange={handleChange} placeholder={isLogin && !isForgotPassword ? "Số điện thoại hoặc Email" : "Số điện thoại"} className="w-full pl-10 pr-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white outline-none focus:ring-2 focus:ring-[#00704A]/50 focus:border-[#00704A] transition-all" required />
                      </div>
                      ...
                      {/* Mật khẩu - ẩn khi Quên mật khẩu (step 1) */}
                      {!isForgotPassword && (
                        <div className="relative">
                      ...
                      {/* Link Quên mật khẩu - chỉ hiện khi Login */}
                      {isLogin && !isForgotPassword && (
                      ...
                      {/* reCAPTCHA - hiện khi Đăng ký hoặc Quên mật khẩu */}
                      {(!isLogin || isForgotPassword) && (
                    ...
                    <div className="space-y-4">
                        ...
                        {/* Nhập mật khẩu mới - chỉ hiện khi Quên mật khẩu + đã nhập OTP */}
                        {isForgotPassword && (
                          <div className="relative">
                            <Lock className="absolute left-3 top-1/2 -translate-y-1/2 size-5 text-white/30" />
